resource "aws_instance" "mastodon-instance-debian-12" {
  ami           = "ami-0db11ba8498cef3a2"
  instance_type = "t2.micro"

  network_interface {
    network_interface_id = aws_network_interface.mastodon-instance-debian-12-eni.id
    device_index         = 0
  }
  ...
  iam_instance_profile = aws_iam_instance_profile.mastodon-instance-debian-12-profile.name
...
echo "vm.swappiness=$${SWAPPINESS}" | sudo tee -a /etc/sysctl.conf
  ...
  tags = {
    Name = "mastodon-instance-debian-12"
  }

}

resource "aws_network_interface" "mastodon-instance-debian-12-eni" {
  subnet_id       = aws_subnet.private-mastodon.id
  security_groups = [aws_security_group.main-security-group.id]

  tags = {
    Name = "mastodon-instance-eni-debian-12"
  }
}

resource "aws_iam_instance_profile" "mastodon-instance-debian-12-profile" {
  name = "mastodon-instance-debian-12-profile"
