...
COMMAND_ID=$( \
  aws ssm send-command --document-name "AWS-RunRemoteScript" \
             ...
             --parameters '{"sourceType":["GitHub"],"sourceInfo":["{\"owner\" : \"iMichka\", \"repository\":\"aws\", \"getOptions\":\"branch:'"$GITHUB_SHA"'\", \"path\":\"prd/ansible-nat-instance/reverse-proxy.yaml\"}"], "commandLine":["ansible-playbook -i “localhost,” reverse-proxy.yaml"]}'\
